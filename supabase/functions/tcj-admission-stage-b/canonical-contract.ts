...
export function validateDiagnosis(value:any):Diagnosis{
  ...
  const ratings:Record<string,number>={};for(const k of DIMS){if(!Number.isInteger(value[k])||value[k]<1||value[k]>4)throw new Error(`bad_${k}`);ratings[k]=value[k];}
  ...
  const confidence=String(value.confidence||'').toLowerCase();if(!['high','medium','low'].includes(confidence))throw new Error('bad_confidence');return{ratings,severity,flags,rationale,confidence};
}
