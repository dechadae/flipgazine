...
Deno.serve(async(req:Request)=>{
  ...
  if(!INTERNAL_KEY)return out({error:'service_unavailable'},503);
  ...
  try{
    const r=await fetch(TARGET,{method:'POST',headers:{authorization,'content-type':'application/json','x-tcj-internal-key':INTERNAL_KEY,...(apikey?{apikey}:{})},body:JSON.stringify(body)});
