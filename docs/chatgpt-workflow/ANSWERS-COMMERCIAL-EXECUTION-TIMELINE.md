...
- Commercial strategy: `ANSWERS-COMMERCIAL-BENCHMARK-PLAN.md`
- Current Batch 2 production architecture: `ANSWERS-BATCH2-CORPUS-ROUTING-REACHABILITY-IMPLEMENTATION-REPORT.md`
...
When checkpoint wording conflicts, use the newest document that owns that decision. Integrity gates outrank calendar targets.
...
B2-0001–0020 answer IDs                   949–968
B2-0021–0040 answer IDs                   969–988
effective Batch 2 source scenarios           1,000
internal buyer-export snapshots                  2
```

B2-0001–0020 remain canonical Book content and qualitative technical-pilot evidence, but are excluded from the clean benchmark denominator.
...
Every ordinary unit is exactly 20 active rows:
...
## Standing preparation-leak rule — unchanged

If machine evidence for an active row is exposed before its first human decision:

```text
STOP that row
→ retire exposed version from active production
→ preserve private append-only provenance
→ create a genuinely new scenario
→ freeze replacement before generation
→ generate new raw draft
→ run new sealed machine evidence
→ verify zero human decisions / zero active exposure exception
→ return only clean replacement to review queue
```
...
# 5. Count model governing commercialization — unchanged
...
The technical first 20 are never silently converted into clean metric rows. B2R is a separate clean supplemental unit and must never overwrite or renumber B2-0001–0020.

If B2R remains benchmark-only, the final evidence count may be described as 1,968 unique reviewed records, but **1,948 remains the canonical Book corpus**.
...
→ buyer checkpoint
→ repeat verified 20-row cycles through B2-1000
→ freeze + complete B2R clean supplemental 20
→ clean 1,000 benchmark freeze
→ final analysis
→ buyer package
...
OpenAI and Thai/SEA conversations remain active in parallel with production. There is no OpenAI exclusivity dependency.

Do not send generic chase messages. The next proactive contact should be a substantive progress/completion update or a response to an actual buyer message.

First-contact/private-evidence policy remains:

```text
ordinary HTTPS links / email body only unless recipient accepts another method
no unsolicited ZIP or HTML attachment
